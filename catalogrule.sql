...
ALTER TABLE `catalogrule_customer_group`
    DROP COLUMN `row_id`;
ALTER TABLE `catalogrule_customer_group`
    ADD FOREIGN KEY (`rule_id`) REFERENCES `catalogrule` (`rule_id`) ON DELETE CASCADE ON UPDATE RESTRICT;
...
ALTER TABLE `catalogrule`
    MODIFY COLUMN `rule_id` INT(10) NOT NULL AUTO_INCREMENT;
ALTER TABLE `catalogrule`
    DROP FOREIGN KEY CATALOGRULE_RULE_ID_SEQUENCE_CATALOGRULE_SEQUENCE_VALUE;

DROP TABLE `magento_banner_catalogrule`;
